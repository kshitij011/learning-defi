...
contract CSAMM {
    ...
    function _burnShares(address _from, uint _amount) private {
        require(balanceOf[_from] >= _amount, "Shares exhausted!");
    ...
    // Amount and address of the token user wants to sell.
    function swap(address _tokenIn, uint _amountIn) external returns(uint amountOut){
        require(_tokenIn == address(token0) || _tokenIn == address(token1), "Address unsupported");
        ...
        // for swap in sum amm dx = dy (amount of token in = amount of token out)
        // amountOut = amountIn;    // 0 fees
        amountOut = (amountIn * 997) / 1000;    // increasing decimal for scaling
    ...
    function addLiquidity(uint _amount0, uint _amount1) external returns(uint shares){
        ...
        // actual amount of tokens came in
        uint d0 = bal0 - reserve0;
        uint d1 = bal1 - reserve1;

        /*
            a = amount in (both tokens)
            L = total liquidity
            s = shares to mint
            T = total supply
